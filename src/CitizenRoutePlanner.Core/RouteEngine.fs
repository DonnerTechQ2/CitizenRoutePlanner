...
module RouteEngine =
    ...
    let estimateTravelTime (fromLoc: LocationInfo) (toLoc: LocationInfo) (locations: LocationIndex) : float =
    ...
    let private createUnknownLocation (destNameOpt: string option) (zoneHostId: uint64) =
        let name = destNameOpt |> Option.defaultValue (sprintf "Unknown Location (%d)" zoneHostId)
        let hashBytes = System.Security.Cryptography.MD5.HashData(System.Text.Encoding.UTF8.GetBytes(name))
        {
            Uuid = Guid(hashBytes)
            Name = name
            Type = "Unknown"
    ...
    let private groupActionsToStops (nodes: RouteNode list) (startLocOpt: LocationInfo option) (locations: LocationIndex) : RouteStop list =
        if List.isEmpty nodes then []
            ...
            while i < nodes.Length do
                ...
                let time = 
                    ...
                    | Some curr -> estimateTravelTime curr loc locations
                    | None -> 0.0
                
                stops.Add({
                    Location = loc
                    Actions = Seq.toList actions
    ...
    let private solveBranchAndBound (nodes: RouteNode list) (startLoc: LocationInfo option) (capacity: int) (currentCargo: int) (locations: LocationIndex) =
                ...
                for node in remaining do
                    ...
                    if not isDropoffButNeedsPickup then
                        ...
                        if newCargo <= capacity then
                            let travelTime = 
                                match lastLoc with
                                | Some l -> estimateTravelTime l node.Location locations
                                | None -> 0.0
    ...
    let private solveGreedy (nodes: RouteNode list) (startLoc: LocationInfo option) (capacity: int) (currentCargo: int) (locations: LocationIndex) =
            ...
            if List.isEmpty validNodes then
                ...
                let bestNode = 
                    validNodes |> List.minBy (fun n -> 
                        match currentLoc with
                        | Some l -> estimateTravelTime l n.Location locations
                        | None -> 0.0)
        ...
        if List.isEmpty nodes then None
            ...
            let optimizedNodes = 
                if nodes.Length <= 12 then
                    solveBranchAndBound nodes startLoc appState.ShipCapacityScu appState.CurrentCargoScu locations
                else
                    solveGreedy nodes startLoc appState.ShipCapacityScu appState.CurrentCargoScu locations
            
            let stops = groupActionsToStops optimizedNodes startLoc locations
            let totalTime = stops |> List.sumBy (fun s -> s.TravelTimeEstimate)
